...
We use `set.seed(1)` and the `createDataPartition`'s caret function to select your test index to create two separate data frames called: `train` and `test` from 
the original `edx` data frame. `test` contain a randomly selected 10% of the 
rows of `edx`, and have `train` contain the other 90%. We will use these data frames to do the rest of the analyses. 
