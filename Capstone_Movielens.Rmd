...
We can see the number of unique users from `train` that provided ratings and how many unique movies were rated:
...
$$
```{r}
mu_hat <- mean(train$rating) # compute mean rating
mu_hat

naive_rmse <- RMSE(test$rating, mu_hat) # compute root mean standard error
naive_rmse
```
...
In this particular situation, we know that the least square estimate $\hat{b}_i$ is just the average of $Y_{u,i} - \hat{\mu}$ for each movie $i$. So we can compute them this way (we will drop the `hat` notation in the code to represent estimates going forward):
...
Regularization permits us to penalize large estimates that are formed using small sample sizes. In this case, the long dataset prevents obscure movies with just a few users,increase our RMSE, so the regularization did not reduce the RMSE. However, we will explore where we made mistakes in our first model, using only movie effects $b_i$. Let's look at the top 10 worst and best movies based on $b_i$ and look at how often they are rated. First, let's create a database that connects movieId to movie title:
...
But, we repeat, in this case the noise is neligible because of a lot of data.

#### Choosing the penalty terms

We use regularization for the estimate both movie and user effects as well. We are minimizing:
